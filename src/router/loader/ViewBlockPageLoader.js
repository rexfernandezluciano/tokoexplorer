...
const ViewBlockPageLoader = async ({ params }) => {
	...
	try {
		...
		if (res.data?.status === "ok") {
			...
			const transactionHashes = operations.flatMap(
				op => op.transactions.flatMap(tx => tx), // Flatten the nested array
			);

			// Create an array of promises to fetch each transaction
			const transactionPromises = transactionHashes.map(hash => client.get(`/transactions/${hash}`));
...
export default ViewBlockPageLoader;
